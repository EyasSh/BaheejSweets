...
namespace BaheejBackEnd.Controllers
{
    ...
    public class RequestController: Controller
    {
        ...
        [HttpPost]
        public async Task HandleRecieveRequestFromClient( HttpContext context) 
        {
            if (context.Request.Path=="/CreateRequest")
            {
                if(context.WebSockets.IsWebSocketRequest)
                {
                    var HandShake= await context.WebSockets.AcceptWebSocketAsync();
                    await Recieve(context, HandShake);
                }
                else
                {
        ...
        private async Task Recieve(HttpContext context, WebSocket ws)
        {
